...
def get_input_file(demo, issue:int = None) -> str:
	s = str(issue).rjust(2,'0') if issue is not None else ''
	f = 'input{}' if not demo else 'input-demo{}'
	return f.format(s)

def get_output_file(demo, issue:int = None) -> str:
	s = str(issue).rjust(2,'0') if issue is not None else ''
	f = 'output{}' if not demo else 'output-demo{}'
	return f.format(s)
